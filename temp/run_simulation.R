...
#Simulate_NHD <- function(n.i, n.t, pop, FITscreen, FITage, FSscreen, FSage, FITthresh, model, risk, sex) { 
 ...
 # m.E[, 1] <- replace(m.E[, 1], m.E[, 1] <=(-0.594), -0.594) # if EQ-5D under -0.594 reset to -0.594
  ...
  for(t in 1:n.t) {
    ...
    # 1- no cancer, 2- LG cancer, 3 -HG cancer, 4 -OC mortality
    
    # Record the characteristics of onset
    m.Diag[, "new_onset"] <-0
    ...
    m.M_8s[, t+1] <- replace(m.M_8s[, t+1], m.M_8s[, t] ==8, 8) #Replace with BC death those who died with BC
    ...
    pop[m.M[, t+1] != 4, "age"] <- pop[m.M[, t+1] != 4 ,"age"] +1
    
    pop[m.M[, t+1] != 4, "risk"] <- calc.CRC.risk(pop[m.M[, t+1] != 4, ])
    f.risk.calc
  }