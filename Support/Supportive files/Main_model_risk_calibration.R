...
source("Support//Supportive files//risk_calibration.R")
...
#mult.smoke=1
n.run <- 3
mult.smoke <- runif(n.run, 1, 2)
mult.past.smoke <- runif(n.run, 0.3, 1.1)
outcome <- matrix(ncol=2, nrow=n.run)
outcome <- cbind(mult.smoke, mult.past.smoke, outcome)
colnames(outcome) <- c("mult.smoke", "mult.past.smoke", "predict_smoke", "predict_past.smoke")
...
#mult.past.smoke= 1.33

  mult.smoke=outcome[i,"mult.smoke"]
  mult.past.smoke= outcome[i,"mult.past.smoke"]
...
outcome[i,"predict_smoke"] <- risk[1]
outcome[i,"predict_past.smoke"] <- risk[2]

write.csv(outcome, file="R calibration// Outputs //risk_calibr_outcome.csv")
